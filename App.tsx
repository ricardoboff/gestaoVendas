...
import { LayoutDashboard, Users, LogOut, Menu, X, Diamond, Shield, Loader2, Settings, Download, HelpCircle, Share, PlusSquare, Receipt } from 'lucide-react';
...
const App: React.FC = () => {
  ...
  return (
    <div className="min-h-screen bg-gray-950 flex flex-col md:flex-row text-gray-100">
      ...
      <aside className={`fixed inset-y-0 left-0 z-30 w-64 bg-gray-900 border-r border-gray-800 transform transition-transform duration-200 ease-in-out md:relative md:translate-x-0 ${mobileMenuOpen ? 'translate-x-0' : '-translate-x-full'}`}>
        <div className="p-6 h-full flex flex-col">
          <div className="hidden md:block mb-8 pl-2"><Logo /></div>
          ...
          <div className="space-y-2 flex-1">
            ...
            <button
              ...
              className={`w-full flex items-center gap-3 px-4 py-3 rounded-xl transition-colors ${currentView === 'customers' || currentView === 'detail' ? 'bg-gray-800 text-white font-medium border border-gray-700' : 'text-gray-400 hover:bg-gray-800 hover:text-white'}`}
            >
              <Users size={20} /> Clientes
            </button>

            <button
              onClick={() => { setCurrentView('expenses'); setMobileMenuOpen(false); }}
              className={`w-full flex items-center gap-3 px-4 py-3 rounded-xl transition-colors ${currentView === 'expenses' ? 'bg-gray-800 text-white font-medium border border-gray-700' : 'text-gray-400 hover:bg-gray-800 hover:text-white'}`}
            >
            ...
            {user.role === 'admin' && (
              <button
                ...
                className={`w-full flex items-center gap-3 px-4 py-3 rounded-xl transition-colors ${currentView === 'admin' ? 'bg-gray-800 text-white font-medium border border-gray-700' : 'text-gray-400 hover:bg-gray-800 hover:text-white'}`}
              >
            ...
            <button
              ...
              className={`w-full flex items-center gap-3 px-4 py-3 rounded-xl transition-colors ${currentView === 'profile' ? 'bg-gray-800 text-white font-medium border border-gray-700' : 'text-gray-400 hover:bg-gray-800 hover:text-white'}`}
            >
            ...
            <button
              ...
              className={`w-full flex items-center gap-3 px-4 py-3 rounded-xl transition-colors ${deferredPrompt ? 'bg-primary text-black font-bold animate-pulse' : 'text-gray-400 hover:bg-gray-800'}`}
            >
          ...
          <div className="mt-auto pt-6 border-t border-gray-800">
            <div className="flex items-center gap-3 px-4 mb-4">
              <div className="w-8 h-8 rounded-full bg-primary text-black flex items-center justify-center font-bold">{user.name.charAt(0)}</div>
              <div className="overflow-hidden">
                <div className="font-medium text-sm truncate text-white">{user.name}</div>
                <div className="text-xs text-gray-500 truncate flex items-center gap-1">{user.role === 'admin' && <Shield size={10} />} @{user.username}</div>
              </div>
            </div>
            <button onClick={handleLogout} className="w-full flex items-center gap-3 px-4 py-2 text-rose-500 hover:bg-rose-900/20 rounded-lg transition-colors text-sm font-medium">
              <LogOut size={18} /> Sair
            </button>
          </div>
        </div>
      ...
      <main className="flex-1 p-4 md:p-8 overflow-y-auto bg-gray-950 relative">
        ...
        <div className="max-w-5xl mx-auto">
          ...
          {currentView === 'admin' && user.role === 'admin' && <AdminPanel currentUser={user} />}
          {currentView === 'profile' && <UserProfile user={user} />}
          {currentView === 'install-help' && <InstallHelp />}
        </div>
      </main>
    </div>
  );
